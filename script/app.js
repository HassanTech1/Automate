...
function startAnimation() {
  if (!animationStarted) {
    ...
    function changeImage() {
      ...
      var imageIndex;
      if (windowWidth <= 768) {
        imageIndex = Math.floor(scrollPercentage * 15 * 54);
      } else {
        imageIndex = Math.floor(scrollPercentage * 3 * 54);
      }
      ...
      scale =
        ...
        (endScale - startScale) *
          (scrollPercentage < 0.5
            ? 50 * scrollPercentage * scrollPercentage
            : -1 + (6 - 3 * scrollPercentage) * scrollPercentage);
    } else {
      scale =
        startScale +
        (endScale - startScale) *
